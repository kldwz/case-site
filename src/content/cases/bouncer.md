---
name: Bouncer
一句话: WhatsApp自动化对话管理平台，整合团队收件箱、销售CRM和AI工作流。
创始人地区: 由 Afif Rus 创立，MY，2025 年成立
营收模式: 订阅制（具体档位未披露）
月收入估算: $3,990/月（Stripe 验证）
流量来源: SEO 自然流量为主（域名评分 DR1），未披露 受众；官网 https://bouncer.my/
可迁移点: ① WhatsApp生态的商业化工具存在需求 ② 轻量级SaaS可通过Stripe快速验证支付 ③ 整合AI工作流可提升产品溢价空间
原文链接: https://bouncer.my/
数据口径: 营收数据来自 TrustMRR 对接的支付服务商 API（Stripe）验证，最后同步于 2026-09-15T11:00:41.374Z；域名评分 DR1；原始页
  https://trustmrr.com/startup/bouncer。
分类: Sales / 未披露 / 海外
类型: 收入案例
证据等级: 官方披露
平台数据: 支付验证：Stripe · 当前 MRR $3,990 · 活跃订阅 46 · 近12个月营收 $20,737 · 累计营收 $21,034 · 域名
  DR1 · 国家 MY · 创立 2025-10-22
封面: /case-site/cases/bouncer/site.webp
---


![Bouncer 官网](/cases/bouncer/site.webp)

# Bouncer：WhatsApp自动化对话管理平台，整合团队收件箱、销售CRM和AI工作流。

## 产品是什么

提供WhatsApp对话自动化管理的SaaS工具，包含统一团队收件箱、销售CRM和AI驱动的工作流功能。

## 怎么赚的钱

通过订阅制模式获得收入，具体定价策略未披露。Stripe验证其MRR为$3,990。

## 流量从哪来

未官方披露，创始人Twitter账号有20,047粉丝可能作为渠道之一。

## 站长是谁

马来西亚开发者Afif Rus，创立于2025年10月。

## 这个案例能学到什么

验证了在主流通讯工具（WhatsApp）上构建商业自动化工具的可行性，展示如何通过轻量化SaaS实现月均$3k+收入。

## 来源与数据

- TrustMRR 原始页：`https://trustmrr.com/startup/bouncer`
- 营收由 Stripe API 验证，非自报；最后同步 2026-09-15T11:00:41.374Z。
- 官网：`https://bouncer.my/`

## 一句话总结

> 马来西亚独立开发者通过WhatsApp自动化工具实现稳定月收入，证明垂直场景SaaS的可行性。
